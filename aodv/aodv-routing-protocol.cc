...
#include <regex>     // For std::regex, std::smatch, and std::regex_search
#include <algorithm>
#include <limits>
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    {
        // START: >>>>>>>>>> Integrated Blockchain-Aware Packet Origination Logic <<<<<<<<<<
        
        // A valid route exists. Before sending the packet, we interact with the blockchain
        // to register the path and tag the packet with a secret and unique ID.

        // 1. Gather necessary routing information from the AODV routing table entry
        Ipv4Address nextHopIp = rt.GetNextHop();
        std::string neighborEthAddress = m_ipToEthAddressMap[nextHopIp];
        int32_t current_packet_hops = rt.GetHop();

        // 2. Generate a random 32-byte secret for this packet
        ...
        // 3. Construct and execute the blockchain transaction to initiate the chain
        std::stringstream cmd_ss;
        cmd_ss << "cast send 0x5FbDB2315678afecb367f032d93F642f64180aa3 "
               << "\"makeChain(uint256,bytes32,address,uint256)\" "
               << current_packet_hops << " " << hex_str << " "
               << neighborEthAddress << " 0 " // ID is 0 for initial creation by the contract
               << "--private-key " << m_ethereumPrivateKey << " "
               << "--rpc-url http://127.0.0.1:8545";

        NS_LOG_WARN("Executing transaction command for packet to " << dst);
        std::string commandOutput = executeCommand(cmd_ss.str().c_str());
        NS_LOG_WARN("RouteOutput signed transaction Output: " << commandOutput);

        // 4. Parse the transaction receipt to find the packet ID emitted by the smart contract event
        const std::string packet_created_topic = "0x5bf72efc39cb0896f6b3f8dfc752687df9672d53f71e33496a5d0c117c6a13a6";
        std::regex topics_regex("\"topics\":\\s*\\[([^\\]]+)\\]");
        std::smatch topics_match;
        uint32_t real_packet_id = 0;
        std::string search_area = commandOutput;

        while (std::regex_search(search_area, topics_match, topics_regex))
        {
            std::string topics_content = topics_match[1].str();
            if (topics_content.find(packet_created_topic) != std::string::npos)
            {
                std::regex id_regex(",\\s*\"(0x[0-9a-fA-F]{64})\"");
                std::smatch id_match;
                if (std::regex_search(topics_content, id_match, id_regex) && id_match.size() > 1)
                {
                    std::string id_hex = id_match[1].str();
                    real_packet_id = std::stoul(id_hex, nullptr, 16);
                    NS_LOG_INFO("Parsed Packet ID from topic: " << real_packet_id);
                    break; // Exit loop once found
                }
            }
            search_area = topics_match.suffix().str();
        }

        // 5. Create and add the tag with the secret and parsed ID to the packet
        SecretForDestinationTag tag;
        tag.SetSecret(hex_str);
        tag.SetId(real_packet_id);
        p->AddPacketTag(tag);
        NS_LOG_WARN("Attached tag to packet. Secret: " << hex_str << ", ID: " << tag.GetId());

        // END: >>>>>>>>>> Integrated Blockchain-Aware Packet Origination Logic <<<<<<<<<<

        // Continue with standard AODV route handling
        route = rt.GetRoute();
        NS_ASSERT(route);
    ...
    // routed to loopback, received from loopback and passed to RouteInput.
    uint32_t iif = (oif ? m_ipv4->GetInterfaceForDevice(oif) : -1);
    DeferredRouteOutputTag tag(iif);
    NS_LOG_DEBUG("Valid Route not found, deferring route request.");
                            ...
                            ErrorCallback ecb)
{
    ...
    // Create a mutable copy of the packet to handle potential tag modifications
    ...
    if (idev == m_lo)
    {
        ...
        {
            DeferredRouteOutput(packetCopy, header, ucb, ecb);
            return true;
        }
    }
         ...
         ++j)
    {
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            ...
            {
                if (m_dpd.IsDuplicate(packetCopy, header))
                {
                    NS_LOG_DEBUG("Duplicated packet " << packetCopy->GetUid() << " from " << origin
                                                      << ". Drop.");
                    return true;
                }
                ...
                if (header.GetTtl() > 1)
                {
                    ...
                    {
                        NS_LOG_DEBUG("No route to forward broadcast. Drop packet " << packetCopy->GetUid());
                    }
                }
                else
                {
                    NS_LOG_DEBUG("TTL exceeded. Drop packet " << packetCopy->GetUid());
                }
                return true;
            }
        }
    ...
    // START: >>>>>>>>>> Integrated Blockchain-Aware Forwarding Logic <<<<<<<<<<

    RoutingTableEntry rtentry;
    if (m_routingTable.LookupRoute(dst, rtentry))
    {
        // Check for the custom secret tag on the packet
        SecretForDestinationTag tag;
        bool hasTag = packetCopy->PeekPacketTag(tag);

        if (hasTag)
        {
            std::string secret = tag.GetSecret();
            uint32_t id = tag.GetId();
            uint32_t id_to_use = id;
            NS_LOG_WARN("the received id " << id);
            
            if (id == 0)
            {
                NS_LOG_WARN("Forwarder received packet with ID 0. Finding ID from logs...");
                std::string senderEthAddress = m_ipToEthAddressMap[origin];
                std::string paddedSender = "0x" + std::string(24, '0') + senderEthAddress.substr(2);
                std::string paddedReceiver = "0x" + std::string(24, '0') + m_ethereumAddress.substr(2);

                std::stringstream find_id_cmd;
                find_id_cmd << "cast rpc --raw eth_getLogs '[{\"address\":\"0x5FbDB2315678afecb367f032d93F642f64180aa3\","
                            << "\"topics\":[null,"
                            << "\"" << paddedSender << "\",\"" << paddedReceiver << "\"],"
                            << "\"fromBlock\":\"earliest\"}]'";
                
                NS_LOG_INFO("Executing find ID command: " << find_id_cmd.str());
                std::string logOutput = executeCommand(find_id_cmd.str().c_str());
                NS_LOG_INFO("Find ID output: " << logOutput);
                
                std::regex data_regex("\"data\":\\s*\"(0x[0-9a-fA-F]{64})");
                std::smatch match;
                if (std::regex_search(logOutput, match, data_regex) && match.size() > 1) {
                    id_to_use = std::stoul(match[1].str(), nullptr, 16);
                    NS_LOG_WARN("Forwarder parsed packet ID: " << id_to_use);

                    SecretForDestinationTag newTag;
                    newTag.SetSecret(secret);
                    newTag.SetId(id_to_use);
                    packetCopy->ReplacePacketTag(newTag); // Update tag on the packet copy
                } else {
                    NS_LOG_ERROR("Forwarder failed to parse packet ID. Dropping packet.");
                    ecb(packetCopy, header, Socket::ERROR_NOROUTETOHOST);
                    return true;
                }
            }
            
            Ipv4Address nextHopIp = rtentry.GetNextHop();
            std::string nextHopEthAddress = m_ipToEthAddressMap[nextHopIp];
            uint32_t remaining_hops = rtentry.GetHop(); // Hops from this node forward
        ...
        // Standard forwarding action
        UpdateRouteLifeTime(rtentry.GetNextHop(), m_activeRouteTimeout);
        m_nb.Update(rtentry.GetNextHop(), m_activeRouteTimeout);
        Ptr<Ipv4Route> route = rtentry.GetRoute();
        NS_LOG_DEBUG("Forwarding to " << dst << " via " << rtentry.GetNextHop());
        ucb(route, packetCopy, header);
        return true;
    }
    
    // END: >>>>>>>>>> Integrated Blockchain-Aware Forwarding Logic <<<<<<<<<<

    // If no route is found, fall back to the original forwarding function,
    // which should handle route discovery (e.g., sending RREQ).
    return Forwarding(packetCopy, header, ucb, ecb);
}
...
RoutingProtocol::FindSubnetBroadcastSocketWithInterfaceAddress(Ipv4InterfaceAddress addr) const
{
    ...
    return socket;
        ...
        void RoutingProtocol::ReadKeysFromFile(const std::string& filename)
        {
            ...
            while (std::getline(file, line))
            {
                ...
                // Parse the new format: ID,PrivateKey,EthAddress,IPAddress
                    ...
                    std::getline(ss, ip_str, ','))
                {
                    try
                    {
                        uint32_t fileNodeId = std::stoul(id_str);
                        Ipv4Address nodeIp(ip_str.c_str());

                        // Store key and address info for all nodes
                        m_ethereumPrivateKeyMap[fileNodeId] = pk_str;
                        m_ethereumAddressMap[fileNodeId] = addr_str;
                        m_ipToEthAddressMap[nodeIp] = addr_str; // Populate the new map
                        m_ethAddressToIpMap[addr_str] = nodeIp;
                        // If this line is for the current node, store its specific keys
                        if (fileNodeId == myNodeId)
                        {
                            m_ethereumPrivateKey = pk_str;
                            m_ethereumAddress = addr_str;
                        }
                    }
...
RoutingProtocol::DoInitialize()
{
    ...
    // --- START: ADDED CODE FOR AUTOMATIC DEPOSIT ---
        ...
        deposit_cmd << "cast send  0x5FbDB2315678afecb367f032d93F642f64180aa3 \"deposit()\" "
                    ...
                    << "--private-key " << m_ethereumPrivateKey << " "
                    << "--rpc-url http://127.0.0.1:8545";
        std::string depositCommand = deposit_cmd.str();
        NS_LOG_WARN("Executing deposit command: " << depositCommand);
        std::string depositOutput = executeCommand(depositCommand.c_str());

        // Clean up the output for better logging
        if (!depositOutput.empty() && depositOutput.back() == '\n') 
        {
            depositOutput.pop_back();
        }
