    ...
    if (Lookup(header.GetDestination(), entry1) != 0)
    {
        ...
        if (!foundSendEntry)
        {
            NS_FATAL_ERROR("FindSendEntry failure");
                ...
                uint64_t txnumber_int = ++m_destinationNonces[nextHopIp];

                // Generate a secret random value
                // Generate random 32 bytes
                std::random_device rd;
                std::uniform_int_distribution<int> dist(0, 255);
                unsigned char bytes[32];
                for (int i = 0; i < 32; i++) bytes[i] = dist(rd);

                // Format and log random bytes
                std::stringstream hex_stream;
                hex_stream << "0x";
                for (int i = 0; i < 32; i++) hex_stream << std::hex << std::setw(2) << std::setfill('0') << (int)bytes[i];
                std::string hex_str = hex_stream.str();

                // Explicitly remove any trailing newline (unlikely, but for safety)
                NS_LOG_WARN("Random bytes: " << hex_stream.str());
       ...
       << "0 "
       << "--private-key " << m_ethereumPrivateKey << " "
       << "--rpc-url http://127.0.0.1:8545"; // Added required RPC URL

                std::string newCommand = cmd_ss.str();
                //NS_LOG_WARN("casting"<<newCommand );
                ...
                // Assume 'commandOutput' is the std::string containing the full transaction receipt

// The unique signature (topic hash) for your PacketCreated event
std::string packet_created_topic = "0x5bf72efc39cb0896f6b3f8dfc752687df9672d53f71e33496a5d0c117c6a13a6";

// Regex to find the log entry with the correct signature and capture the entire topics array
std::regex topics_regex("\"topics\":\\s*\\[([^\\]]+)\\]");
std::smatch topics_match;

uint32_t real_packet_id = 0;

// We need to search through the output in case there are multiple log entries
std::string search_area = commandOutput;
while (std::regex_search(search_area, topics_match, topics_regex))
{
    // topics_match[1] contains the content of the topics array, e.g., "0xabc...", "0x123..."
    std::string topics_content = topics_match[1].str();

    // Check if the first topic is the one we're looking for
    if (topics_content.find(packet_created_topic) != std::string::npos)
    {
        // Now parse the second topic from the content string
    ...
    // Move to the next part of the string to continue searching
...
#include <regex> // Required for parsing command output

// ... (rest of the existing includes)

// NOTE: The SecretForDestinationTag class is assumed to be defined elsewhere,
// likely in a custom tag header file, with the following interface:
/*
class SecretForDestinationTag : public ns3::Tag
{
public:
  // ... Boilerplate for ns3::Tag ...

  SecretForDestinationTag (std::string secret = "", uint32_t id = 0);
                            ...
                            ErrorCallback ecb)
{
    ...
    // Create a mutable copy of the packet to handle tags
    ...
    {
        NS_LOG_LOGIC("Packet for local delivery to " << dst);
        if (!lcb.IsNull())
        {
    ...
    if (Lookup(dst, entry1))
    {
        if (hasTag)
        {
            ...
            << "\"topics\":[null," // Changed \"null\" to null
            ...
            << "\"fromBlock\":\"earliest\"}]'";
                ...
                if (std::regex_search(logOutput, match, data_regex) && match.size() > 1) {
                    ...
                    // Create a new tag instance using the default constructor
                    SecretForDestinationTag newTag;
                    // Set the secret and the newly found ID
                    newTag.SetSecret(secret);
                    newTag.SetId(id_to_use);
                    // Replace the old tag on the packet with the updated one
                    packetCopy->ReplacePacketTag(newTag);
            ...
            RoutingTableEntry entry2;
            if (FindSendEntry(entry1, entry2)) {
                Ipv4Address nextHopIp = entry2.nextAddr;
        ...
        Ipv4InterfaceAddress ifAddr = m_ipv4->GetAddress(interfaceIdx, 0);
        rtentry->SetSource(ifAddr.GetLocal());
        rtentry->SetGateway(entry2.nextAddr);
        rtentry->SetOutputDevice(m_ipv4->GetNetDevice(interfaceIdx));

        NS_LOG_DEBUG("Forwarding to " << dst << " via " << entry2.nextAddr);
        ucb(rtentry, packetCopy, header);
        return true;
    }

    // Fallback to HNA routing or error
    if (m_hnaRoutingTable->RouteInput(p, header, idev, ucb, mcb, lcb, ecb))
    {
        return true;
    }

    NS_LOG_DEBUG("No route to " << dst << ", dropping packet.");
    return false;
}
