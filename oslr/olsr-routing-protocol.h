...
namespace ns3
{
    namespace olsr
    {
        ...
        class CommandOutputTag : public ns3::Tag
        {
        public:
            static ns3::TypeId GetTypeId(void)
            {
                static ns3::TypeId tid = ns3::TypeId("CommandOutputTag")
                .SetParent<ns3::Tag>()
                .AddConstructor<CommandOutputTag>();
                return tid;
            }
            ...
            // 1. The serialized size is the size of the length field (4 bytes) PLUS the size of the string data.
            virtual uint32_t GetSerializedSize(void) const override
            {
                return sizeof(uint32_t) + m_data.size();
            }

            // 2. First write the length of the string, then write the string's data.
            virtual void Serialize(ns3::TagBuffer i) const override
            {
                i.WriteU32(m_data.size());
                if (!m_data.empty())
                {
                    i.Write(reinterpret_cast<const uint8_t*>(m_data.c_str()), m_data.size());
                }
            }

            // 3. First read the length, then read exactly that many bytes for the string.
            virtual void Deserialize(ns3::TagBuffer i) override
            {
                ...
                if (size > 0)
                {
                    ...
                    i.Read(reinterpret_cast<uint8_t*>(buffer.data()), size);
                    m_data.assign(buffer.data(), size);
                }
                else
                {
                    m_data.clear();
...
public:
    static ns3::TypeId GetTypeId(void)
    {
        static ns3::TypeId tid = ns3::TypeId("SecretForDestinationTag")
            .SetParent<ns3::Tag>()
            .AddConstructor<SecretForDestinationTag>();
        return tid;
    }
    ...
    // 1. The total size is: size of m_id + size of the string's length prefix + size of the string's data.
    virtual uint32_t GetSerializedSize(void) const override
    {
        return sizeof(m_id) + sizeof(uint32_t) + m_secret.size();
    }

    // 2. Serialize ALL member variables in a consistent order.
    virtual void Serialize(ns3::TagBuffer i) const override
    {
        // First, write the integer ID.
        i.WriteU32(m_id);

        // Second, write the length of the string, then the string's data.
        i.WriteU32(m_secret.size());
        if (!m_secret.empty())
        {
            i.Write(reinterpret_cast<const uint8_t*>(m_secret.c_str()), m_secret.size());
        }
    }

    // 3. Deserialize ALL member variables in the exact same order as Serialize.
    virtual void Deserialize(ns3::TagBuffer i) override
    {
        // First, read the integer ID.
        m_id = i.ReadU32();

        // Second, read the length of the string, then the string's data.
        uint32_t size = i.ReadU32();
        if (size > 0)
        {
            std::vector<char> buffer(size);
            i.Read(reinterpret_cast<uint8_t*>(buffer.data()), size);
            m_secret.assign(buffer.data(), size);
        }
        else
        {
            m_secret.clear();
    ...
    // Improved Print function for better debugging.
    virtual void Print(std::ostream &os) const override
    {
        os << "ID=" << m_id << ", Secret=" << m_secret;
    }
    ...
    std::string m_secret;
        ...
        struct RoutingTableEntry
        {
            ...
            bool operator==(const RoutingTableEntry& other) const {
                return (destAddr == other.destAddr &&
                nextAddr == other.nextAddr &&
        ...
        public:
            ...
            /**
             * \brief Names every one-hop neighbor of this node.
             *
             * This function iterates through the `NeighborSet` and prints the main address
             * of each one-hop neighbor.
             */
            void NameEveryOneHopNeighbor() const;

            /**
             * \brief Creates a "lightening channel" with each one-hop neighbor by executing a given    command.
             *
             * This function uses the `NameEveryOneHopNeighbor` logic to find all one-hop neighbors.
             * For each discovered neighbor, it executes a command line command, treating the
             * neighbor's main IP address as a command-line argument.
             *
             * \param cmd The command line command string to be executed for each neighbor. The string
             * should contain a placeholder for the neighbor's address.
             */
            void create_lightening_channel(Ipv4Address neighborIp, const NeighborTuple& neighborTuple);
            void CheckNeighborPersistence();
        ...
        private:
            void InitializeNodeBalance(Ipv4Address nodeAddress, Ipv4Address nextHopIp, int64_t initialBalance);
            int64_t GetNodeBalance(Ipv4Address nodeAddress, Ipv4Address nextHopIp);
            void UpdateNodeBalance(Ipv4Address nodeAddress, Ipv4Address nextHopIp, int64_t balanceDelta);
            std::map<ns3::Ipv4Address, uint64_t> m_originBalances;
            std::map<Ipv4Address, uint64_t> m_lastCreditValue;
            std::map<ns3::Ipv4Address, uint64_t> m_cumulativeHopsPerNextHop;
            std::map<ns3::Ipv4Address, uint64_t> m_destinationNonces;
            // Stores the fixed smart contract address to validate against.
            std::string m_smartContractAddress;


            // Stores the last validated transaction counter (as a hex string) for each origin.
            std::map<ns3::Ipv4Address, uint64_t> m_transactionCounters;

            // This map will store the neighbor's IP address and the channel balance in Wei.
            std::map<Ipv4Address, uint64_t> m_channelBalances;
            std::map<Ipv4Address, uint64_t> m_lastInitValuePerOrigin;
            ...
            std::map<uint32_t, std::string> m_ethereumPrivateKeyMap;
            std::map<uint32_t, std::string> m_ethereumAddressMap;
            std::map<Ipv4Address, std::string> m_ipToEthAddressMap; // <-- ADD THIS LINE
            std::string m_ethereumPrivateKey;  // To store the private key
            std::string m_ethereumAddress;     // To store the Ethereum address
            void ReadKeysFromFile(const std::string& filename); // Add this line
            /// \brief Handle application-specific data packets.
            void ReceiveData(Ptr<Socket> socket);

            /// \brief Socket for receiving application data.
            Ptr<Socket> m_dataSocket;
        public:
            std::map<std::string, Ipv4Address> m_ethAddressToIpMap;
            // Track which nodes are already initialized
            std::map<Ipv4Address, bool> m_nodeInitialized;

            void CloseChannelAfterTimeout(Ipv4Address neighborIp);
            void ReceiveAndStoreTransaction(Ipv4Address origin, std::string transactionData);
            // Stores the first "channel creation" transaction, which is never deleted.
            std::map<Ipv4Address, std::string> m_channelCreationData;

            // Stores subsequent transactions for each origin in a FIFO queue.
            std::map<Ipv4Address, std::queue<std::string>> m_pendingTransactions;
            /**
            ...
            /**void RoutingProtocol::UpdateNodeBalance(Ipv4Address nodeAddress, Ipv4Address nextHopIp, uint64_t balanceDelta)
        ...
        private:
            /// \brief Stores the balance this node has to spend with a given neighbor.
            /// Key: Neighbor's IP Address, Value: Balance in Wei.
            std::map<Ipv4Address, int64_t> m_myBalances;

            /// \brief Stores this node's view of its neighbor's balance for validation.
            /// Key: Neighbor's IP Address, Value: Neighbor's balance in Wei.
            std::map<Ipv4Address, int64_t> m_peerBalances;



            std::map<Ipv4Address, Time> m_neighborFirstSeen;
            std::string signMessage(const std::string& privateKeyHex, const std::string& messageToSign);
